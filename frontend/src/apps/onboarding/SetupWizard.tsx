...
export const SetupWizard: React.FC<SetupWizardProps> = ({
  ...
  // Step 3: Restaurant Size
  const [tablesCount, setTablesCount] = useState<number>(10);

  const ownerName = currentUser?.name || initialOwnerData?.ownerName || initialOwnerData?.name || 'Restaurant Owner';
  const ownerEmail = currentUser?.email || initialOwnerData?.ownerEmail || initialOwnerData?.email || 'owner@dinely.app';
  ...
  const handleNextStep = () => {
    ...
    if (currentStep === 1) {
      ...
      if (!address.trim() || !city.trim() || !country.trim()) {
        setErrorMessage('Please enter the street address, city, and country.');
        return;
      }
      setCurrentStep(3);
  ...
  const handleSubmitApplication = async () => {
    ...
    try {
      // 1. Ensure owner user exists in session
      let user = api.getCurrentUser();
      if (!user) {
        throw new Error('Owner user session not found. Please sign in or create an account first.');
      }

      // 2. Create new restaurant record linked to owner
      ...
      // 3. Submit application for Platform Admin Approval with table count
      ...
      <main className="max-w-2xl w-full mx-auto my-8 relative z-10">
        <Card className="bg-slate-900/90 border-slate-800/90 p-6 sm:p-10 backdrop-blur-xl shadow-2xl space-y-8 rounded-3xl">
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400">Enter your venue name and select your restaurant type.</p>
              </div>

              <div className="space-y-5">
                <Input
                  label="Restaurant Name *"
                  placeholder="e.g. Cafe.Co or Lumiere Bistro"
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 bg-indigo-950/40 border border-indigo-500/30 rounded-2xl flex items-start gap-3 text-xs text-slate-300">
                ...
                <div>
                  ...
                  Upon approval by Platform Admin, Dinely will automatically create <span className="font-bold text-rose-400">{tablesCount} tables</span> with table-specific QR codes ready for immediate customer dining.
                </div>
              </div>
            </div>
          )}
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <div className="space-y-1">
                <Badge variant="brand" className="mb-1">Step 4</Badge>
                <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
              ...
              <div className="p-6 bg-slate-950 rounded-2xl border border-slate-800 space-y-4">
                <div className="grid grid-cols-2 gap-4 text-xs">
                  ...
                  <div className="col-span-2 pt-2 border-t border-slate-800/80">
                    ...
                    <span className="text-slate-400 text-[11px] block">{ownerEmail}</span>
                  </div>
                </div>
              </div>
            </div>

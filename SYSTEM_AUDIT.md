...
**Status:** FORENSIC AUDIT COMPLETE & HARDENED — CORE ROOT CAUSES RESOLVED  
**Target Environment:** Production Platform (`https://dinely.food`) | Backend (`https://dineflow-v3.onrender.com/api/v1`) | Database (Neon PostgreSQL) | Auth (Google Firebase) | Cloud Run (Prepared with Guardrails)

---

## 1. Executive Summary

A comprehensive, layer-by-layer forensic audit of the Dinely multi-tenant restaurant SaaS platform was conducted across all 32 architectural and operational dimensions. Core findings and remediations:

1. **Single Source of Truth Enforced:** The PostgreSQL database is strictly authoritative for restaurant identity, lifecycle status (`LIVE`, `PENDING_APPROVAL`, `REJECTED`, `SUSPENDED`), and tenant ownership. Stale `localStorage` and client memory can NEVER override or downgrade live approved restaurants to "Opening Soon".
2. **Eradication of Production Synthetic Auth:** Eliminated synthetic `df_jwt_google_` token generation in `frontend/src/packages/api/client.ts`. Production authentication strictly demands cryptographic Google Firebase ID tokens validated server-side.
3. **Canonical Domain & Universal QR Encoding:** Canonical format `https://<slug>.dinely.food/customer?table=01&tableId=<id>` is universally generated by both frontend (`getRestaurantCustomerUrl`) and backend (`generate_canonical_qr_url`). Zero `?tenant=` or `.dinely.app` dependencies exist in active paths.
4. **WebSocket Security & Token Refresh Safeguard:** Realtime connections automatically refresh expired Firebase tokens on close code 1008 and enforce bounded retry backoff (max 4 immediate attempts on persistent auth rejection) with zero raw token exposure in logs.
5. **Pre-Billing Guardrails & Cloud Run Preparation:** Hardened deployment script (`scripts/setup_gcp_guardrails.py`) prepared for Google Cloud Run (min 0, max 3 instances, 80 concurrency, 3600 timeout, dedicated least-privilege service account). Google Cloud billing activation on project `dinely-cd6cd` is clearly tracked as a pending human account action while Render remains 100% active and operational as fallback.

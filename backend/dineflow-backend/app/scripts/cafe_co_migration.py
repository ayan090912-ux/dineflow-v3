...
async def run_migration():
    ...
    async with AsyncSessionLocal() as db:
        # 2. Seed CAFE.CO for all active restaurant IDs
        target_ids = ["rest-1787655544312", "rest-1787446097984", "rest-1"]
        for rest_id in target_ids:
            ...
            categories_data = [
                (f"cat-{rest_id}-1", "Starters & Appetizers", 1),
                (f"cat-{rest_id}-2", "Main Course", 2),
                (f"cat-{rest_id}-3", "Gourmet Desserts", 3),
                (f"cat-{rest_id}-4", "Beverages & Drinks", 4),
            ]
            ...
            # 4. Seed Menu Items
            items_data = [
                (f"item-{rest_id}-1", f"cat-{rest_id}-1", "Truffle Mushroom Arancini", "Crispy risotto balls stuffed with wild forest mushrooms and aged mozzarella, served with truffle aioli.", 14.50, "https://images.unsplash.com/photo-1541529086526-db283c563270?w=600", True, "KITCHEN"),
                (f"item-{rest_id}-2", f"cat-{rest_id}-2", "Pan-Seared Salmon Fillet", "Atlantic salmon served over saffron risotto with lemon herb reduction.", 28.90, "https://images.unsplash.com/photo-1467003909585-2f8a72700288?w=600", False, "KITCHEN"),
                (f"item-{rest_id}-3", f"cat-{rest_id}-3", "Classic Tiramisu", "Layers of espresso-soaked ladyfingers and whipped mascarpone cream.", 9.50, "https://images.unsplash.com/photo-1571877227200-a0d98ea607e9?w=600", True, "KITCHEN"),
                (f"item-{rest_id}-4", f"cat-{rest_id}-4", "Artisanal Smoked Old Fashioned", "Bourbon whiskey infused with aromatic bitters and oak wood smoke.", 16.00, "https://images.unsplash.com/photo-1514933651103-005eec06c04b?w=600", True, "BAR"),
            ]
            ...
            await db.commit()
            print(f"[PASS] Migrated restaurant {rest_id} (categories, menu items, tables)")
